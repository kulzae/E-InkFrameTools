...
Simple Image Server is a lightweight Python HTTP server that feeds random images to e-ink photoframes URL mode. Each frame's image_url names the folder to serve from, and the server picks a random image from that folder on every rotation request, never repeating the last image it sent. Requires Pillow for automatic image rotation for portrait/landscape orinentations
